...
class GameOfLife():
    def __init__(self,blocksize,max_gen):
        pygame.init()
        self.blocksize = blocksize
        
        
        self.window = pygame.display.set_mode((1920,1080), DOUBLEBUF|FULLSCREEN)
        self.window.set_alpha(None)

        pygame.display.flip()
        self.w, self.h = pygame.display.get_surface().get_size()
        self.field = np.array([[0 for j in range(int(self.w/self.blocksize))] for i in range(int(self.h/self.blocksize))])
        self.next_field = np.array([[0 for j in range(int(self.w/self.blocksize))] for i in range(int(self.h/self.blocksize))])
    ...
    def draw_grid(self):
        for y in range(int(self.w/self.blocksize)):
            for x in range(int(self.h/self.blocksize)):
                self.draw_sq(y,x)
